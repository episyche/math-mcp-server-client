#!/usr/bin/env python3
"""
YouTube System Rules Demo - Working with actual MCP server
"""
...
class YouTubeAssistant:
    """YouTube assistant that uses the actual MCP server."""
    ...
    async def start_server(self):
        ...
        try:
            ...
            print("🚀 Started YouTube MCP server")
            return True
    ...
    async def stop_server(self):
        """Stop the YouTube MCP server."""
        if self.server_process:
            self.server_process.terminate()
            self.server_process.wait()
            print("🛑 Stopped YouTube MCP server")
    
    async def call_server_tool(self, tool_name, arguments):
        """Call a tool on the YouTube MCP server."""
        try:
            # Create a new subprocess for each call to avoid connection issues
            ...
            return {"error": f"Server call failed: {str(e)}"}
    
    async def list_videos(self, user_id):
        """Call the actual list_videos tool on the server."""
        return await self.call_server_tool("list_videos", {"user_id": user_id})
    
    async def search_videos(self, user_id, query):
        """Call the actual search_videos tool on the server."""
        return await self.call_server_tool("search_videos", {"user_id": user_id, "query": query})
    
    async def get_channel_analytics(self, user_id):
        """Call the actual get_channel_analytics tool on the server."""
        return await self.call_server_tool("get_channel_analytics", {"user_id": user_id})
    ...
    async def process_query(self, query):
        ...
        if "search" in query.lower():
            ...
            result = await self.search_videos(user_id, search_query)
            ...
            result = await self.get_channel_analytics(user_id)
        else:
            ...
            result = await self.list_videos(user_id)
        
        # Rule 3: Always return structured results from tools
        return result

async def main():
    """Main demo function."""
    print("🚀 YouTube System Rules Demo with Actual MCP Server")
    print("=" * 60)
    ...
    print("6. ✅ Uses actual MCP server for real tool calls")
